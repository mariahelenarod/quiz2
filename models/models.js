...
sequelize.sync().then(function() {									// sequelize.sync() inicializa tabla de preguntas en DB
	Quiz.count().success(function(count) {							// success ejecuta el manejador cuando crea la tabla. Quiz.count().success() devuelve el numero de filas
		if (count === 0) {											// se inicializa solo si esta vacia
			Quiz.create({
				pregunta: DataTypes.STRING,
		  		respuesta: DataTypes.STRING
			}).success(function() {console.log('Base creada')});
		};
	});
});